...
ScoreLibrary.ScoreDiv.prototype.initContextCache = function(div_node) {
    ...
    this.canvas_link_node = $('<a target="_blank" href="#"/>');

    this.canvas_link_node.appendTo(this.div_node);

    this.canvas_node.appendTo(this.canvas_link_node);
...
ScoreLibrary.ScoreDiv.prototype.setCanvasLink = function(data_url) {

    this.canvas_link_node.prop('href', data_url);
};
...
ScoreLibrary.ScoreDiv.prototype.showCurrPage = function() {
    ...
    if (this.page_iterator.hasNext()) { // !NOTE: Really ask 'has current?'
        ...
        this.setCanvasLink(draw_context.toDataURL());
