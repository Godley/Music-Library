...
Objects will be necessary to avoid coupling, but there are two other proposed options. The first is applying an XSL stylesheet to the XML file. This is not possible because of the complexity of music, though this is the usual method for rendering an XML file in browser.
The second more viable option is a converter script, which would take in MusicXML files and directly output to the output method of the project. This has not been chosen for two reasons, the first being that this would couple the project with MusicXML, and any new input formats would have to create a completely new script, rather than converting to a standard object structure which would do conversion to output formats. The second reason is that conversion from input to output in this manner would have the big O representation of $O(n^2)$, whilst a conversion of input to object, and object to any output would have the representation of $O(n)$.
